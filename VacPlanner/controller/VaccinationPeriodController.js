...
module.exports = class VaccinationPeriodController {
  ...
  parseDate(reservationDate) {
    ...
    if (newDate.isValid()) {
      ...
      const month =
        (newDate.month() + 1).toString().length == 1
          ? "0" + (newDate.month() + 1)
          : newDate.month() + 1;
      const day =
        newDate.date().toString().length == 1
          ? "0" + newDate.date()
          : newDate.date();
  ...
  async realocateReservations(date, turn, vp, amountToAdd) {
    ...
    let listReservations = await this.countryDataAcces.getReservations(
      zone_id,
      state_code,
      `${parseDate} 00:00:00`,
      `${parseDate} 23:59:59`,
    ...
    let criteria = JSON.parse(
      await this.countryDataAcces.getACriteria(vp.assignment_criteria_id)
    )[0];
      ...
      if (amountToReturn > 0) {
        ...
        if (person) {
          if (fun(person)) {
            ...
            await axios
              .post("http://localhost:5007/sms/", object)
              .then()
  ...
  async addVaccinationPeriod(body) {
    let vp = JSON.parse(
      await this.countryDataAcces.addVaccinationPeriod({
    ...
    for (var i = 0; i < turns; i++) {
      ...
      newAmountMat = await this.realocateReservations(
        auxDate,
        i % 2 ? 3 : 2,
        vp,
        newAmountMat
  ...
  async updateAVaccinationPeriod(id, name) {
    let vpJson = JSON.parse(
      await this.countryDataAcces.getAVaccinationPeriod(id)
    )[0];
    let oldAmount;
    ...
    if (name.vaccine_amount) {
      if (vp) {
        ...
        for (let i = 0; i < turns; i++) {
          ...
          let slot = JSON.parse(
            await this.slotController.getASlot({
          ...
          if (slot.available_slots == 0 && amountToAddTotal != 0) {
            amountToAdd = await this.realocateReservations(
              startDate,
              i % 2 ? 3 : 2,
              vpJson,
              amountToAdd
