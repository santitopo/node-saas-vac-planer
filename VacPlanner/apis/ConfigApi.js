...
module.exports = class ConfigApi {
  ...
  init() {
    ...
    const vaccinationPeriodController = new VaccinationPeriodController(this.countryDataAccess, slotController)
    ...
    router.get("/vaccinationperiods/:id", async (ctx, next) => {
      const res = await vaccinationPeriodController.getAVaccinationPeriod(ctx.params.id)
      ctx.response.body = res
    });
    ...
    router.delete("/vaccinationperiods/:id", async (ctx, next) => {
      const res = await vaccinationPeriodController.deleteAVaccinationPeriod(ctx.params.id)
      if (res) {
        ctx.response.body = "Eliminada satisfactoriamente"
      } else {
        ctx.response.body = "No se pudo eliminar, el codigo no existe"
    ...
    router.put("/states/:code", async (ctx, next) => {
      const res = await stateController.updateAState(ctx.params.code, ctx.request.body)
      if (res) {
    ...
    router.put("/zones/:id", async (ctx, next) => {
      const res = await zoneController.updateAZone(ctx.params.id, ctx.request.body)
      if (res) {
        ctx.response.body = "Nombre modificado correctamente"
      } else {
        ctx.response.body = "No se pudo modificar, el codigo no existe"
    ...
    router.put("/vaccenters/:id", async (ctx, next) => {
      const res = await vacCenterController.updateAVacCenter(ctx.params.id, ctx.request.body)
      if (res) {
        ctx.response.body = "Nombre modificado correctamente"
      } else {
        ctx.response.body = "No se pudo modificar, el codigo no existe"
    ...
    router.put("/vaccines/:id", async (ctx, next) => {
      const res = await vaccineController.updateAVaccine(ctx.params.id, ctx.request.body)
      if (res) {
    ...
    router.put("/vaccinationperiods/:id", async (ctx, next) => {
      const res = await vaccinationPeriodController.updateAVaccinationPeriod(ctx.params.id, ctx.request.body)
      if (res) {
        ctx.response.body = "Modificado correctamente"
      } else {
        ctx.response.body = "No se pudo modificar, el codigo no existe"
